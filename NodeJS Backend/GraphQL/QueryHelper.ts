...
export class QueryHelper {
    ...
    public async sendQuery<T>(query_str:string): Promise<T> {
    ...
    public async getPools(first: number, orderBy:string, orderDirection:OrderDirection ): Promise<Pool[]>
    {
        ...
        while(first_count_down > 0) {
            ...
            const query_str:string = QueryBuilder.Pools(adjusted_first, skip_amount, orderBy, orderDirection);
            // send the query, store the promise, don't wait, move on
            promises.push(this.sendQuery<Pool>(query_str));
    ...
    /* Gets a list of Uniswap V3 Pools. Any number of them instead of just 1000 */
    public async getTokens(first: number, orderBy:string, orderDirection:OrderDirection ): Promise<object[]>
    {
        // Gonna call a bunch of async functions, without using await,
        // stuff them into a list called promises and then wait until they are all filled:
        ...
        while(first_count_down > 0) {
            ...
            const skip_amount = 1000 * i;
            // generate a graphql query string:
            const query_str:string = QueryBuilder.Tokens(adjusted_first, skip_amount, orderBy, orderDirection);
            // send the query, store the promise, don't wait, move on
    ...
    orderDirection:OrderDirection ): Promise<TokenDayData[]> {
        // Gonna call a bunch of async functions, without using await,
        // stuff them into a list called promises and then wait until they are all filled:
        let promises: Promise<TokenDayData>[] = []
        // 'first' is the variable passed into the query, like first: 10, first: 100 etc
        // it normally can only go to 1000.
        let first_count_down:number = first;
        let i:number = 0;
        while(first_count_down > 0) {
            ...
            const query_str:string =
                QueryBuilder.TokenDayDatas(adjusted_first, skip_amount,orderBy,orderDirection)
            // send the query, store the promise, don't wait, move on
            promises.push(this.sendQuery<TokenDayData>(query_str));
            // if this results in a negative number, we are done:
            first_count_down -= 1000;
            i++;
        }
    ...
    public async getTokenHourData(
        ...
        // Gonna call a bunch of async functions, without using await,
        // stuff them into a list called promises and then wait until they are all filled:
        let promises: Promise<TokenHourData>[] = []
        // 'first' is the variable passed into the query, like first: 10, first: 100 etc
        // it normally can only go to 1000.
        let first_count_down:number = first;
        let i:number = 0;
        while(first_count_down > 0) {
            ...
            // generate a graphql query string:
            const query_str:string =
                QueryBuilder.TokenHourDatas(adjusted_first, skip_amount,orderBy,orderDirection)
            // send the query, store the promise, don't wait, move on
            promises.push(this.sendQuery<TokenHourData>(query_str));
            // if this results in a negative number, we are done:
            first_count_down -= 1000;
            i++;
        }
    ...
    * this runs them all at once:*/
    public async getPoolsByID(ids_list:string[]): Promise<Pool[]> {
        ...
        while (ids.length > 0) {
            ...
            let query_str: string = QueryBuilder.Pools(undefined, undefined,undefined, undefined, ids_fragment);
