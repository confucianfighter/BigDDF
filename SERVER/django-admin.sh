...
# to the SERVER/settings.py INSTALLED_APPS[] list.
...
''
# Missing some django dependencies for my project to work.
# Do I mount the image? Is it already mounted? Aren't I specifying djangorestframework?
# Going to install locally, get the version number and change that in requirements.txt
# source venv/bin/activate
# still got the problem, restarting vscode
# going to uninstall then reinstall django and djangorestframework
# did a whole bunch of stuff. Finally, deleted venv
# ran python3 -m venv ./venv
# switched to bash because this wouldn't run in zsh:
# >source ./venv/bin/activate
# shows (venv) bash prompt
# must pip install all venv stuff from bash!
